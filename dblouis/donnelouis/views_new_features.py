...
def toggle_article_like(request, article_id):
    ...
    try:
        ...
        article = Article.objects.get(id=article_id)
        type_vote = int(request.POST.get('type_vote', 1))  # 1 ou -1
        ...
        if existing_like:
            # Si c'est le même type de vote, supprimer
...
def get_article_likes(request, article_id):
    """Récupérer le nombre de likes/dislikes d'un article"""
    try:
        from .models import Article, ArticleLike
...
# ============================================================================
# API pour récupérer les contenus les plus aimés
# ============================================================================

@require_http_methods(["GET"])
def get_popular_articles(request):
    """API: Articles les plus aimés"""
    from .models import Article, ArticleLike
    from django.db.models import Count
    
    popular = Article.objects.annotate(
        like_count=Count('likes', filter=Q(likes__type_vote=1))
    ...
    data = [
        {
            ...
            'titre': a.titre,
            'slug': a.slug,
            'likes': a.likes.filter(type_vote=1).count(),
        }
        for a in popular
    ]
...
@require_http_methods(["GET"])
def get_popular_forum_sujets(request):
    """API: Sujets forum les plus aimés"""
    from .models import ForumSujet, ForumSujetLike
    from django.db.models import Count
    
    popular = ForumSujet.objects.annotate(
        like_count=Count('likes', filter=Q(likes__type_vote=1))
    ).order_by('-like_count')[:10]
    ...
    data = [
        {
            ...
            'titre': s.titre,
            'slug': s.slug,
            'likes': s.likes.filter(type_vote=1).count(),
        }
        for s in popular
    ]
    
    return JsonResponse({'success': True, 'sujets': data})